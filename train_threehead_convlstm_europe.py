...
OUT_DIR   = "/gpfs/home2/mzdych/thesis/three_head_output_medi_2003_all_coeff"
...
# ── Year splits ───────────────────────────────────────────────────────────
TRAIN_YEARS = list(range(1990, 2001))   # 1990–2000
VAL_YEARS   = list(range(2001, 2003))   # 2001–2002
TEST_YEARS  = list(range(2003, 2004))   # 2003

# ── Input features ────────────────────────────────────────────────────────
# COEFFS = ["BC", "DC", "ID", "OD", "is_heatwave", "CC"]   # CN only
# COEFFS = ["swvl1", "land_mask", "u", "v", "z"]            # ERA5 only
COEFFS = ["BC", "DC", "ID", "OD", "CC", "is_heatwave",
...
BATCH_SIZE = 4
HIDDEN_DIM = 32
N_EPOCHS   = 100   # high ceiling — early stopping will kick in
...
for ax, key, title in zip(axes,
    ...
    ax.axvline(best_epoch - 1, color="gray", linestyle="--", label=f"best epoch {best_epoch}")
...
with open(os.path.join(OUT_DIR, "metrics.txt"), "w") as f:
    ...
    f.write(f"Best epoch : {best_epoch}  (early stopping patience={ES_PATIENCE})\n\n")
